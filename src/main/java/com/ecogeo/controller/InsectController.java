...
public class InsectController {
  ...
  @PostMapping("/species_result_insect.ajax")
  public String resultThree(@RequestParam String filter, @RequestParam List<String> group_name, @RequestParam List<String> item_group
    ...
    Map<String,List<InsectItem>> selectItem = itemService.selectInsectItem(items);
    List<InsectItem> have = selectItem.get("have");
    String none = "";
    int noneCount = 0;

    for(InsectItem s : selectItem.get("none")) {
      none += s.getRealName() + "\r\n";
      noneCount++;
    }
    ...
    Map<String,List<InsectItem>> order = new HashMap<>();

    //먼저 OrderName으로 분류
    for(InsectItem i : have) {
      if(order.containsKey(i.getOrderName())) {
        order.get(i.getOrderName()).add(i);
      } else {
        List<InsectItem> itemList = new ArrayList<>();
        itemList.add(i);
    ...
    for(String key : order.keySet()) {
      ...
      List<InsectItem> orderList = order.get(key);
      for(InsectItem i : orderList) {
  ...
  @Data
  public static class ItemDTO extends InsectItem {
    List<String> group;

    public void pack(InsectItem i) {
